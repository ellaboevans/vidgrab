...
import { RiAppleFill, RiWindowsFill, RiTerminalBoxLine } from "@remixicon/react";
...
const PLATFORMS = [
  {
    ...
    features: ["Native Performance", "Gatekeeper Compatible", "Sign & Notarized"],
  },
  {
    name: "Windows",
    icon: RiWindowsFill,
          ...
          {PLATFORMS.map((platform, idx) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="group relative p-8 rounded-xl border border-border bg-gradient-to-br from-card/80 to-card/40 hover:from-card to-card/60 transition-all duration-300 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10"
              >
                {/* Accent gradient top-right */}
                <div className="absolute -top-px -right-px w-32 h-32 bg-gradient-to-br from-primary/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-full filter blur-2xl" />

                <div className="relative z-10">
                  {/* Icon */}
                  <div className="w-16 h-16 rounded-lg bg-gradient-to-br from-primary/30 to-secondary/20 flex items-center justify-center mb-6 group-hover:from-primary/50 group-hover:to-secondary/40 transition-all">
                  ...
                  <button className="mt-8 w-full py-3 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary font-semibold transition-all border border-primary/30 hover:border-primary/50 group/btn relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-r from-primary/0 via-primary/20 to-primary/0 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
